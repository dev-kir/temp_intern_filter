...
Matches the exact SARI_Organisation.xlsx format.
...
def headers(ws, row, n):
    for c in range(1, n + 1):
        x = ws.cell(row, c)
        x.fill = PatternFill("solid", fgColor=BLUE)
        x.font = Font(bold=True, color=WHITE)
...
def calculate(df, cfg):
    ...
    for org in sorted(df["Organisation name"].unique()):
        for sec in sections:
            ...
            mx = scores.max()
            maxp = maxs.mean()
            norm = avg / maxp if maxp else None
        ...
        if qid in multi:
            ...
            for _, x in g.iterrows():
                for opt in str(x["Answer"] or "").split(","):
                    if opt.strip():
                        pairs.append((x["Respondent ID"], opt.strip(), x["Answer score"]))
...
def build(df, odf, sdf, qdf, ddf, cfg, out):
    ...
    title(ws, "SARI Organisation Statistics",
          "Generated from the complete raw Answers export. No peer comparison or automated narrative is included.", 8)
    notes = [
        ("Input", "Source workbook Answers sheet."),
        ("Refresh", "Run the generator again against the latest complete export."),
        ("Deduplication", "Respondent ID + Question ID; last record retained."),
        ("Agreement", "High >=80%; Moderate >=60%; Low <60%; Not measurable below two respondents."),
        ("Review flag", "Consensus below 60% where agreement is measurable."),
        ("Scoring", "Only Max score > 0 contributes to maturity."),
        ("Privacy", "Email is omitted from Raw Answers."),
    ]
    for r, (a, b) in enumerate(notes, 4):
        ws.cell(r, 1, a).font = Font(bold=True, color=NAVY)
        ws.cell(r, 2, b)
    ...
    title(ws, "Organisation Dashboard", "Select an organisation to view the current results.", 10)
    ws["A4"] = "Selected organisation"
    ws["B4"] = odf.iloc[0]["Organisation name"] if len(odf) else ""
    ...
    items = [
        ("A6", "Respondents", "A7", "C"),
        ("D6", "Overall score", "D7", "J"),
        ("G6", "Strongest section", "G7", "K"),
        ("A9", "Weakest section", "A10", "L"),
        ("D9", "Agreement", "D10", "O"),
        ("G9", "Interpretation", "G10", "P"),
        ("A12", "Maturity tier", "A13", "Q"),
        ("D12", "Distance to next tier", "D13", "R"),
        ("G12", "Questions for review", "G13", "N"),
    ]
    for lc, label, vc, col in items:
        ws[lc] = label
        ws[lc].font = Font(bold=True, color=NAVY)
        ws[vc] = f'=IFERROR(INDEX(\'Organisation Summary\'!${col}:${col},MATCH($B$4,\'Organisation Summary\'!$A:$A,0)),"")'
        ws[vc].fill = PatternFill("solid", fgColor=LIGHT)
        ws[vc].font = Font(size=13, bold=True, color=NAVY)
        ws[vc].alignment = Alignment(wrap_text=True)
    ...
    for r, s in enumerate(sections, 18):
        ws.cell(r, 1, s)
        ws.cell(r, 2, f'=IFERROR(SUMIFS(\'Section Summary\'!$E:$E,\'Section Summary\'!$A:$A,$B$4,\'Section Summary\'!$B:$B,$A{r}),"")')
        ws.cell(r, 3, f'=IFERROR(SUMIFS(\'Section Summary\'!$J:$J,\'Section Summary\'!$A:$A,$B$4,\'Section Summary\'!$B:$B,$A{r}),"")')
        ws.cell(r, 4, f'=IFERROR(SUMIFS(\'Section Summary\'!$C:$C,\'Section Summary\'!$A:$A,$B$4,\'Section Summary\'!$B:$B,$A{r}),"")')
        ws.cell(r, 5, f'=IFERROR(LOOKUP(2,1/((\'Section Summary\'!$A$5:$A${4+len(sdf)}=$B$4)*(\'Section Summary\'!$B$5:$B${4+len(sdf)}=$A{r})),\'Section Summary\'!$L$5:$L${4+len(sdf)}),"")')
        ws.cell(r, 3).number_format = "0.0%"

    ch = BarChart()
    ch.type = "bar"
    ch.title = "Section maturity profile"
    ch.add_data(Reference(ws, min_col=3, min_row=17, max_row=24), titles_from_data=True)
    ch.set_categories(Reference(ws, min_col=1, min_row=18, max_row=24))
    ch.height = 7
    ch.width = 12
    ch.legend = None
    ws.add_chart(ch, "G17")
    ws.freeze_panes = "A17"
    for c, w in {"A": 35, "B": 18, "C": 18, "D": 22, "E": 18, "F": 4, "G": 28, "H": 18, "I": 18, "J": 18}.items():
        ws.column_dimensions[c].width = w

    # ── 4. Organisation Report ──
    ws = wb.create_sheet("Organisation Report")
    title(ws, "SARI Organisation Report", "Select an organisation. The page is formatted for printing.", 8)
    ws["A3"] = "Selected organisation"
    ws["B3"] = odf.iloc[0]["Organisation name"] if len(odf) else ""
    ...
    items = [
        ("A5", "Overall score", "B5", "J"),
        ("D5", "Maturity tier", "E5", "Q"),
        ("G5", "Respondents", "H5", "C"),
        ("A7", "Strongest section", "B7", "K"),
        ("D7", "Weakest section", "E7", "L"),
        ("G7", "Agreement", "H7", "O"),
    ]
    for lc, label, vc, col in items:
        ws[lc] = label
        ws[lc].font = Font(bold=True, color=NAVY)
        ws[vc] = f'=IFERROR(INDEX(\'Organisation Summary\'!${col}:${col},MATCH($B$3,\'Organisation Summary\'!$A:$A,0)),"")'
        ws[vc].fill = PatternFill("solid", fgColor=LIGHT)
        ws[vc].alignment = Alignment(wrap_text=True)
    ws["B5"].number_format = "0.0%"

    for c, h in enumerate(["Section", "Score", "Agreement"], 1):
        ws.cell(10, c, h)
    headers(ws, 10, 3)
    for r, s in enumerate(sections, 11):
        ws.cell(r, 1, s)
        ws.cell(r, 2, f'=IFERROR(SUMIFS(\'Section Summary\'!$J:$J,\'Section Summary\'!$A:$A,$B$3,\'Section Summary\'!$B:$B,$A{r}),"")')
        ws.cell(r, 3, f'=IFERROR(LOOKUP(2,1/((\'Section Summary\'!$A$5:$A${4+len(sdf)}=$B$3)*(\'Section Summary\'!$B$5:$B${4+len(sdf)}=$A{r})),\'Section Summary\'!$L$5:$L${4+len(sdf)}),"")')
        ws.cell(r, 2).number_format = "0.0%"

    ws["A20"] = "Priority questions for review"
    ws["A20"].font = Font(size=12, bold=True, color=NAVY)
    for c, h in enumerate(["Question ID", "Question", "Most common answer", "Normalised score", "Agreement", "Review flag"], 1):
        ws.cell(21, c, h)
    headers(ws, 21, 6)
    for i, r in enumerate(range(22, 27), 1):
        for c, col in enumerate(["B", "C", "D", "E", "F", "G"], 1):
            ws.cell(r, c, f'=IFERROR(INDEX(\'Priority Detail\'!${col}:${col},MATCH($B$3&"|{i}",\'Priority Detail\'!$I:$I,0)),"")')
        ws.cell(r, 4).number_format = "0.0%"

    for c, w in enumerate([30, 50, 34, 18, 18, 18, 18, 18], 1):
        ws.column_dimensions[get_column_letter(c)].width = w
    ...
    data_sheets = [
        ("Organisation Summary", "One row per organisation. Sort or filter by respondent count, score or agreement.",
         odf, [42, 31, 12, 22, 22, 18, 25, 18, 14, 14, 34, 34, 18, 20, 18, 30, 20, 20]),
        ("Section Summary", "Section-level maturity and internal agreement for each organisation.",
         sdf, [42, 38, 12, 12, 14, 14, 14, 14, 14, 18, 18, 18]),
        ("Question Summary", "Question-level statistics. Use Review flag to find low-consensus scored questions.",
         qdf, [42, 34, 18, 60, 12, 16, 38, 18, 14, 14, 14, 14, 14, 14, 18, 18, 18, 16]),
        ("Answer Distribution", "Counts and percentages by answer option. Multi-select totals can exceed 100%.",
         ddf, [42, 34, 18, 60, 18, 42, 14, 20, 14, 20]),
    ]

    for name, sub, frame, widths in data_sheets:
        ws = wb.create_sheet(name)
        title(ws, name, sub, len(frame.columns))
        write_table(ws, list(frame.columns), frame.where(pd.notna(frame), None).values.tolist(), widths)
    ...
    title(ws, "Raw Answers", "Imported source rows with Standard section added. Email is omitted.", len(RAW_OUT))
    write_table(ws, RAW_OUT, rframe.where(pd.notna(rframe), None).values.tolist(),
                [22, 18, 34, 12, 18, 60, 42, 18, 14, 14, 28, 24, 42, 31, 18, 31, 25, 16, 24, 28, 14, 16, 26])
